...
pub async fn get_txn_by_no(pool: &SqlitePool, txn_no: &str) -> Result<Option<TxnRow>, AppError> {
  let row = sqlx::query(
    "SELECT id, txn_no, type, occurred_at, created_at, operator_id, item_id, from_slot_id, to_slot_id, qty, actual_qty, ref_txn_id, note \
     FROM txn WHERE txn_no = ?",
  )
  .bind(txn_no)
  .fetch_optional(pool)
  .await?;
...
pub async fn has_reversal(pool: &SqlitePool, ref_txn_id: &str) -> Result<bool, AppError> {
  let (count,): (i64,) = sqlx::query_as(
    "SELECT COUNT(1) FROM txn WHERE ref_txn_id = ? AND type = 'REVERSAL'",
  )
  .bind(ref_txn_id)
  .fetch_one(pool)
  .await?;
...
pub async fn list_txns(
  ...
  let mut builder: QueryBuilder<Sqlite> = QueryBuilder::new(
    "SELECT txn.id, txn.txn_no, txn.\"type\" AS txn_type, txn.occurred_at, txn.created_at, \
     ...
     ref.txn_no AS ref_txn_no, ref.\"type\" AS ref_txn_type, ref_it.name AS ref_item_name, \
     ref_op.display_name AS ref_operator_name, ref_fs.code AS ref_from_slot_code, \
     ref_ts.code AS ref_to_slot_code, ref.qty AS ref_qty, ref.actual_qty AS ref_actual_qty, \
     ref.occurred_at AS ref_occurred_at, ref.note AS ref_note, \
     txn.note \
     FROM txn \
  ...
  if let Some(item_code) = item_code {
    push_where(&mut builder);
    builder.push("it.item_code = ");
    builder.push_bind(item_code);
  }

  if let Some(operator_name) = operator_name {
    push_where(&mut builder);
    builder.push("op.display_name = ");
    builder.push_bind(operator_name);
  }

  if let Some(slot_code) = slot_code {
    push_where(&mut builder);
    builder.push("(fs.code = ");
    builder.push_bind(slot_code.clone());
    builder.push(" OR ts.code = ");
    builder.push_bind(slot_code);
    builder.push(")");
  }

  if let Some(warehouse_code) = warehouse_code {
    let pattern = format!("{}-%", warehouse_code);
    push_where(&mut builder);
    builder.push("(fs.code LIKE ");
  ...
  if let Some(rack_code) = rack_code {
    let pattern = format!("%-{}-%", rack_code);
    push_where(&mut builder);
    builder.push("(fs.code LIKE ");
    builder.push_bind(pattern.clone());
    builder.push(" OR ts.code LIKE ");
...
pub async fn list_txns_export(pool: &SqlitePool) -> Result<Vec<TxnExportRow>, AppError> {
  let rows = sqlx::query(
    "SELECT txn.type AS txn_type, item.item_code AS item_code, \
     from_slot.code AS from_slot_code, to_slot.code AS to_slot_code, \
     txn.qty AS qty, txn.actual_qty AS actual_qty, txn.occurred_at AS occurred_at, \
     operator.username AS operator_username, txn.note AS note, ref.txn_no AS ref_txn_no \
     FROM txn \
     JOIN item ON txn.item_id = item.id \
     JOIN operator ON txn.operator_id = operator.id \
     LEFT JOIN slot AS from_slot ON txn.from_slot_id = from_slot.id \
     LEFT JOIN slot AS to_slot ON txn.to_slot_id = to_slot.id \
     LEFT JOIN txn AS ref ON txn.ref_txn_id = ref.id \
