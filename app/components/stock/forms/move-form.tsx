...
import type { MoveFormValues, SlotPickerValue } from "../types";
...
export default function MoveForm({ onClose, form: externalForm  }: Props) {
  const form = externalForm ?? useForm<MoveFormValues>({ defaultValues: { item_id: "", from_slot_id: "", to_slot_id: "", qty: "", occurred_at: "", operator_id: getSession()?.actor_operator_id || "", note: "" } });
  ...
  const [outboundSlotItemsState, setOutboundSlotItemsState] = useState<any[]>([]);

  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      if (!target?.slotId) {
        setOutboundSlotItemsState([]);
        return;
      }
      try {
        setOutboundSlotItemsState(await getItemListBySlotId(target.slotId));
      } catch (err) {
        setOutboundSlotItemsState([]);
      }
    };
  ...
  const submitLocal = async () => {
    try {
      ...
      if (!ok) return false;
      // placeholder: implement create_move
      if (onClose) onClose();
      toast.success("移库提交（本地实现）");
      form.reset({ item_id: "", from_slot_id: "", to_slot_id: "", qty: "", occurred_at: "", operator_id: "", note: "" });
      return true;
    } catch (err) {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <FormField control={form.control} name="from_slot_id" rules={{ validate: (value) => (value ? true : "请选择来源位置") }} render={({ field }) => (
          <FormItem className="grid gap-2 md:col-span-2">
            <FormControl>
              <SlotCascaderPicker label="来源位置" value={source} onChange={(next) => { setLocalSource(next); field.onChange(next.slotId || ""); }} disabled />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="to_slot_id" rules={{ validate: (value) => { if (!value) return "请选择目标位置"; const fromSlot = form.getValues("from_slot_id"); return value === fromSlot ? "来源位置与目标位置不能相同" : true; } }} render={({ field }) => (
          <FormItem className="grid gap-2 md:col-span-2">
            <FormControl>
              <SlotCascaderPicker label="目标位置" value={target} onChange={(next) => { setLocalTarget(next); field.onChange(next.slotId || ""); }} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
